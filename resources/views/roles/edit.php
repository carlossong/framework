...
<div class="py-6 sm:px-0">
    ...
    <div class="max-w-5xl bg-white shadow-sm overflow-hidden sm:rounded-xl border border-slate-200">
        <div class="px-6 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
                <h3 class="text-xl font-bold text-slate-900 line-clamp-1">Editar Função: <span class="text-primary"><?= htmlspecialchars($role['name']) ?></span></h3>
                <p class="mt-1 text-sm text-slate-500">Defina as permissões de acesso para esta função.</p>
            </div>
            <div class="flex items-center space-x-3">
                <span class="text-xs font-medium text-slate-400 uppercase tracking-wider">Status:</span>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800">Ativo</span>
            </div>
        </div>
        ...
        <div class="px-6 py-8">
            ...
            <form action="/roles/<?= $role['id'] ?>/edit" method="POST" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php component('input', ['name' => 'name', 'label' => 'Nome da Função', 'value' => $role['name'], 'required' => true, 'placeholder' => 'Ex: Gerente']); ?>
                    <?php component('input', ['name' => 'description', 'label' => 'Descrição', 'value' => $role['description'] ?? '', 'placeholder' => 'Descreva as responsabilidades desta função']); ?>
                </div>
                                        ...
                                        <?php foreach ($perms as $permission): ?>
                                            <label class="flex items-start cursor-pointer group/label">
                                                <div class="relative flex items-center group">
                                                    <input type="checkbox" id="perm_<?= $permission['id'] ?>" name="permissions[]" value="<?= $permission['id'] ?>" <?= in_array($permission['id'], $rolePermissionIds ?? []) ? 'checked' : '' ?> data-resource="<?= $resource ?>" class="peer sr-only">
